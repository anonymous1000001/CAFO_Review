...
def main(cfg: DictConfig) -> None:
    ...
    # elif cfg.task_name in ['gilon_speed']:
    #         # get exp num
    #     for exp_dir in all_exp_dirs:
    #         exp_num = int(exp_dir.split("/")[-2].split("EXP")[-1])
    #         result_df = pd.read_csv(exp_dir)
    ...
    #         assert result_df["task_name"].iloc[0] == cfg.task_name, "Task name should be same"
    #         # get rmse, mse, mae
    #         rmse = mean_squared_error(result_df["y_true"], result_df["y_pred"], squared=False)
    #         mse = mean_squared_error(result_df["y_true"], result_df["y_pred"], squared=True)
