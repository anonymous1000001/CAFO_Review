...
@hydra.main(version_base=None, config_path="conf", config_name="ce_config")
def main(cfg: DictConfig) -> None:
    ...
    if cfg.task.task_name in ["gilon_activity", "gilon_bodyweight", "gilon_speed"]:
        log.info(f"Setting up GILON Dataset with CV number {cfg.task.validation_cv_num}")
        from dataset.gilon_dataset import RecbamDataModule
    elif cfg.task.task_name in ["microsoft_activity", "microsoft_activity_new"]:
        log.info(f"Setting up Microsoft Dataset with CV number {cfg.task.validation_cv_num}")
        from dataset.microsoft_dataset import RecbamDataModule
        ...
        from dataset.finger_dataset import RecbamDataModule
    elif cfg.task.task_name == "squid_game":
        log.info(f"Setting up Squid Game Dataset with CV number {cfg.task.validation_cv_num}")
        from dataset.squidgame_dataset import RecbamDataModule
    elif cfg.task.task_name in ["SpokenArabicDigits"]:
        from dataset.uea_dataset import RecbamDataModule
    else:
        NotImplementedError(f"Task {cfg.task.task_name} is not implemented")

    if cfg.lightning_model == "lightning_model_cls_baseline":
        from lightning_models.lightning_model_cls_baseline import LitModel
    elif cfg.lightning_model == "lightning_model_cls_cafo_ce":
    ...
    if cfg.lightning_model in ["lightning_model_reg_recbam", "lightning_model_cls_recbam"]:
    ...
    if cfg.lightning_model in ["lightning_model_reg_recbam", "lightning_model_cls_recbam"]:
        ...
        elif cfg.task.task_name in ["gilon_speed"]:
            from importance_visualizer.gilon_speed_visualizer import GilonSpeedVisualizer

            visualizer = GilonSpeedVisualizer(cfg)
